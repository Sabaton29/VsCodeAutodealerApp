...
const menuButtonClasses = "flex items-center w-full px-4 py-2 text-sm text-left text-gray-300 hover:bg-brand-red hover:text-white rounded-md transition-colors";

const ServiceActions: React.FC<ServiceActionsProps> = ({ service, onEdit, onDelete, onViewDetails, hasPermission }) => {
        ...
        <div className="relative" ref={menuRef}>
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-48 z-10 bg-dark-light border border-gray-700 rounded-lg shadow-xl p-2 space-y-1">
                    <button 
                        onClick={() => onViewDetails && handleAction(onViewDetails)} 
                    ...
                    {hasPermission('manage:services') && (
                        <>
                            ...
                            <button onClick={() => handleAction(onDelete)} className={`${menuButtonClasses} !text-red-400 hover:!bg-red-800/50 hover:!text-white`}>
