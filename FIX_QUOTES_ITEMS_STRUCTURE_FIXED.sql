-- FIX QUOTES ITEMS STRUCTURE (FIXED VERSION)
-- Este script corrige la estructura de la columna items en quotes con casts explícitos
...
-- 2. Ahora que items es un array, corregir los unitPrice con casts explícitos
UPDATE quotes 
SET items = (
    SELECT jsonb_agg(
        CASE 
                ...
                jsonb_set(
                    ...
                    to_jsonb(
                        COALESCE(
                            ...
                            (SELECT i.cost_price FROM inventory_items i WHERE i.id = (item->>'id')::uuid),
                            0
                        )
                    )
                )
