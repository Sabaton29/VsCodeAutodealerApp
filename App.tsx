...
const EnhancedReportUnforeseenIssueModal = lazy(() => import('./components/EnhancedReportUnforeseenIssueModal'));
...
const AppContent: React.FC = () => {
    ...
    const renderActiveView = () => {
        if (viewingWorkOrder) return <Suspense fallback={<LoadingSpinner />}><WorkOrderDetailView 
            ...
            onReportUnforeseenIssue={() => openModal('ENHANCED_REPORT_UNFORESEEN_ISSUE', viewingWorkOrder)}
        ...
        switch (ui.activeView) {
            ...
            case 'Ajustes': return <Suspense fallback={<LoadingSpinner />}><SettingsView 
                ...
                onUpdateAllWorkOrderStages={data.handleUpdateAllWorkOrderStages}
                    onRestoreIncorrectlyCompletedOrders={data.handleRestoreIncorrectlyCompletedOrders}
                    onFixOrdersWithQuoteStageMismatch={data.handleFixOrdersWithQuoteStageMismatch}
            /></Suspense>;
...
const ModalManager: React.FC = () => {
    ...
    return (
        <>
            ...
            <Modal isOpen={type === 'ENHANCED_REPORT_UNFORESEEN_ISSUE'} onClose={closeModal} title={`Reportar Imprevisto en OT #${modalData?.id ?? ''}`} size="xl">
                {modalData && <EnhancedReportUnforeseenIssueModal
                    workOrder={modalData}
                    client={data.clients.find(c => c.id === modalData.client?.id)}
                    vehicle={data.vehicles.find(v => v.id === modalData.vehicle?.id)}
                    onSave={async(issue: UnforeseenIssue) => {
                        await data.handleReportUnforeseenIssue(modalData.id, issue);
                        closeModal();
                    }}
                    onCancel={closeModal}
                    onCreateQuoteFromIssue={(issue: UnforeseenIssue) => {
                        // Convertir el imprevisto en una cotización
                        const quoteItems: QuoteItem[] = [];
                        
                        // Agregar servicios del imprevisto
                        if (issue.requiredServices) {
                            issue.requiredServices.forEach(service => {
                                quoteItems.push({
                        ...
                        if (issue.requiredParts) {
                            issue.requiredParts.forEach(part => {
                                quoteItems.push({
                        ...
                        openModal('CREATE_QUOTE', newQuote);
                    }}
                />}
            </Modal>
