...
const CreateWorkOrderForm: React.FC<CreateWorkOrderFormProps> = ({
    ...
    const vehiclesForClient = useMemo(() => vehicles.filter(v => v.clientId === formState.clientId), [vehicles, formState.clientId]);
            ...
            <div className="flex justify-end -mt-2">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-4">
                 ...
                 <FormSection title="2. Información del Servicio" icon={<Icon name="clipboard" className="w-5 h-5 text-brand-red" />}>
                     <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium mb-1">Servicio Solicitado / Falla Reportada</label>
                        ...
                        <div className="space-y-3 bg-black/10 dark:bg-black/20 p-3 rounded-lg">
                            <ToggleSwitch
                            ...
                            {formState.requiresInitialDiagnosis && (
                                <div className="mt-3 pl-4 border-l-2 border-brand-red/50">
                                    <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Tipo de Diagnóstico Requerido</label>
                                    <div className="flex flex-wrap gap-x-4 gap-y-2">
                                        {(['Básico', 'Intermedio', 'Avanzado'] as DiagnosticType[]).map(type => (
                                            <label key={type} className="flex items-center text-sm text-gray-700 dark:text-gray-300">
                                                <input
                                                    type="radio"
                                                    name="diagnosticType"
                                                    value={type}
                         ...
                         <div>
                            <label className="block text-sm font-medium mb-1">Asesor de Servicio</label>
                ...
                <div className="space-y-6">
                    <FormSection title="3. Estado Detallado del Vehículo" icon={<Icon name="car" className="w-5 h-5 text-brand-red" />}>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-1">Objetos de Valor Reportados</label>
                    ...
                    <FormSection title="4. Checklist de Daños" icon={<Icon name="exclamation-triangle" className="w-5 h-5 text-brand-red" />}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                             ...
                             <div>
                                <label className="block text-sm font-medium mb-1">Descripción de Daños / Observaciones</label>
                                <textarea
                                    name="otherDamages"
                    ...
                    <FormSection title="6. Comentarios y Evidencia" icon={<Icon name="document-text" className="w-5 h-5 text-brand-red" />}>
                        <div>
                            <label className="block text-sm font-medium mb-1">Comentarios Adicionales del Asesor</label>
                            <textarea name="comments" value={formState.comments} onChange={handleChange} rows={3} className={inputBaseClasses}></textarea>
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Evidencia Fotográfica de Ingreso</label>
                            <div className="flex items-center gap-4">
